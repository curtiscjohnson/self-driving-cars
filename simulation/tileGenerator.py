...
for i in range(6):
    lastAngle += gapAngle
    startPoint = (
        (sideLen / 2) * math.cos(lastAngle) + sideLen,
        -(sideLen / 2) * math.sin(lastAngle),
    )
    ...
    corner = cv2.fillPoly(
        corner, np.int32([polyPoints]), yellow, cv2.LINE_AA, fractionalBits
    )
...
toIter = np.arange(
    0, tileInches + step / 2, 3
)  # +step/2 is an ugly hack to get the endpoint included in the range, but whatever.
for i in toIter:
    xCoord = round(i * pixelsPerInch)
    straight = cv2.line(
        straight, (xCoord, 0), (xCoord, sideLen), black, centerPixels, cv2.LINE_AA
    )

# Top and bottom borders
straight = cv2.line(
    straight, (0, borderPixels // 2), (sideLen, borderPixels // 2), white, borderPixels
)
straight = cv2.line(
    straight,
    (0, sideLen - borderPixels // 2),
    (sideLen, sideLen - borderPixels // 2),
    white,
    borderPixels,
)
